...
function Player (config) {
  ...
  // audio node generated by videojs
  this.audioEl = this.audioPlayer.contentEl().getElementsByTagName('audio')[0]
  ...
  this.$bottomBarCover = $('#bottom-bar-cover')
...
Player.prototype.play = function (data, time, isDJ) {
  if (!data) {
    console.log('Play error: song = null')
    return
  }
  if (time == null) time = 0
  if (isDJ == null) isDJ = false
...
Player.prototype.playMp3 = function (meta, time, isDJ) {
  ...
  if (!isDJ) { // is not this user's mp3 => download from peers
    ...
    var tr = self.torrentClient.add(meta.infoHash, function (torrent) {
      self.currentTorrent = torrent
      
      var file = torrent.files[0]
      console.log('started downloading file: ', file)
      
      file.renderTo(self.audioEl)
    ...
    tr.on('done', function(){
      ...
      var file = tr.files[0].getBlob(function (error, blob) {
        ...
        // cover can only be retrieved when mp3 done downloading
...
Player.prototype.playYouTube = function (meta, time) {
  ...
  this.setCover('https://img.youtube.com/vi/' + id + '/mqdefault.jpg')
...
Player.prototype.setCover = function (cover) {
  ...
  if (this.player == null || cover == null) {
    this.player.posterImage.hide()
    this.$bottomBarCover.removeAttr('style')
    return
  }
  var coverUrl = 'url(' + cover + ')'

  if (this.player === this.audioPlayer) {
    this.$audio.find('.vjs-poster').css('background-image', coverUrl)
    this.player.posterImage.show()
  }

  this.$bottomBarCover.css('background-image', coverUrl)
}
